...
A collection of utilities for WSL (Windows Subsystem for Linux) that provides seamless Docker integration and corporate CA certificate management.
...
The installer will:
1. Enable systemd in WSL (if not already enabled)
2. Install Docker Engine and create Windows wrappers
3. Install the `update-corporate-ca` tool to `/usr/local/bin`
4. Set up configuration files

### Installation Options

```bash
# Show detailed debug output during installation
./install.sh --verbose
