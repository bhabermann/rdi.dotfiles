...
SCRIPT_REL="update-corporate-ca/bin/update-corporate-ca"
CONFIG_REL="update-corporate-ca/config/update-corporate-ca.conf"
...
normalize_lf() {
  # Strip CRLF from installed targets (prevents /usr/bin/env bash\r and config $'\r' errors)
  sudo sed -i 's/\r$//' "/usr/local/bin/$PROGRAM"
...
main() {
  # Soft warning if running from /mnt/c (WSL)
